...
int cantidadDeCaracteres_file(FILE* bloque){
	int i; ;
	if(fseek(bloque,0,SEEK_SET)==-1)perror("cantidadDeCaracteres_file()");//lo posiciono desde el inicio
...
//	FILE* aux = fopen(pathFile,"r+");
//	unsigned int posicion= ftell(aux);
//	fseek(aux,0,SEEK_SET);
//	for(cantidad=0;getc(aux)!=EOF;cantidad++);
//	fseek(aux,posicion,SEEK_SET);
//	fclose(aux);
//	return cantidad;
//}
